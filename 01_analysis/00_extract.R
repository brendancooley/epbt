...
# Schott links to data for 2004-2006 currently broken
...
# polity dir
politydir <- paste0(datadir, "polity/")
mkdir(politydir)
...
for (i in seq(startY, endY, 1)) {
  ...
  if (!file.exists(uspath)) {
    if (y > 2003 & y < 2007) {
      schottbase <- "https://faculty.som.yale.edu/peterschott/wp-content/uploads/sites/47/2019/06/"
    }
    else {
      schottbase <- "https://spinup-000d1a-wp-offload-media.s3.amazonaws.com/faculty/wp-content/uploads/sites/47/2019/06/"
    }
    usYurl <- paste0(schottbase, usorigin, ".zip")
    temp <- tempfile()
...
raw_polity <- read.spss("http://www.systemicpeace.org/inscr/p4v2017.sav", to.data.frame = TRUE) %>% as_tibble()
raw_polity$eyear <- as.integer(raw_polity$eyear)
write_csv(raw_polity, paste0(politydir, "polity.csv"))
