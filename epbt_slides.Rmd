...
- $\bm{\theta}$ - structural parameters
- $\bm{L}_{N \times 1}$ - labor endowments

$$
\left\{ \bm{d}, \bm{L}, \bm{\theta} \right\} \rightarrow \left\{ \bm{X}_{N \times N}(\bm{w}), \bm{P}_{N \times 1}(\bm{w}) \right\}
$$

- $\bm{w}_{N \times 1}$ - wages
- $\bm{X}_{N \times N}(\bm{w})$ - trade flows
- $\bm{P}_{N \times 1}(\bm{w})$ - price levels
...
$$
\lambda_{ij} = \frac{ T_j \left( d_{ij} w_j^{1 - \beta} P_j^{\beta} \right)^{- \theta} }{\gamma^{-1} P_i^{-\theta}}
$$
...
$$
\hat{\tau}_{ij} = \left( \frac{\hat{\lambda}_{ij}}{\hat{\lambda}_{jj}} \right)^{-\frac{1}{\theta}} \frac{\hat{P}_i}{\hat{P}_j} \frac{1}{\delta_{ij}(\bm{Z}_{ij})}
$$

- $\frac{\hat{\lambda}_{ij}}{\hat{\lambda}_{jj}}$ - relative expenditure shares
  + Data: trade flows and national accounts
- $\frac{\hat{P}_i}{\hat{P}_j}$ - relative price levels
  + Data: International Comparison Program (World Bank), good prices and exchange rates
  + Estimate preference weights, $\bm{\alpha}$
- $\delta_{ij}(\bm{Z}_{ij})$
  + $\bm{Z}_{ij}$ dyadic geographic covariates
