...
\end{equation}
with $\sigma > 0$. $\tilde{alpha}_{ik} = \epsilon_{ik} \alpha_k$ is a stochastic preference parameter that modulates country $i$'s consumer's relative preference for goods in category $i$. These preferences are constant across consumers up to a shock, $\epsilon_{ik}$, with $\E [ \epsilon_{ik} ] = 1$.
...
P_i = \left( \int_{[0,1]} \tilde{alpha}_{h(\omega)} p_i(\omega)^{1 - \sigma} d \omega \right)^{\frac{1}{1 - \sigma}}
\end{equation}
