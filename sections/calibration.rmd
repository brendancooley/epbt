...
N <- read_csv(paste0("../estimation/", cleandir, "ccodes.csv")) %>% pull(.) %>% length()
P <- read_csv(paste0("../estimation/", cleandir, "priceIndex.csv")) %>% filter(year==Y)
...
I present results from a calibration on a set of `r N` of the world's largest economies in `r Y`.^[The list of the economies in the sample is included in the Appendix.] These in-sample countries collectively made up `r round(gdpSample/gdpWorld*100, digits=0)` percent of world GDP. I treat the rest of the world as an aggregate outside economy. The calibration requires me to take a stance on two structural parameters, the Frechet parameter $\theta$ and the consumers' elasticity of substitution $\sigma$. I set $\sigma - 1 = \theta =$ `r theta`, in line with the estimates from the structural gravity literature [@Head2014].
...
```{r P, echo=FALSE, warning=FALSE, message=FALSE, fig.cap = "Price indices and tradable expenditure shares \\label{fig:P}", fig.height=5, dpi=300, fig.pos="t"}
