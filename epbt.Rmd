...
source("estimation/04_freight.R")
...
I present results from a calibration on a set of `r N` of the world's largest economies in 2005.^[The list of the economies in the sample is included in the Appendix.] These in-sample countries collectively made up `r round(gdpSample/gdpWorld*100, digits=0)` percent of world GDP. I treat the rest of the world as an aggregate outside economy. The calibration requires me to take a stance on one structural parameter, the trade elasticity, $\theta$. I set this parameter equal to `r sigma - 1`, in line with the estimates from the structural gravity literature [@Head2014].
...
```{r P, echo=FALSE, warning=FALSE, message=FALSE, fig.cap = "Price indices and tradable expenditure shares \\label{fig:P}", fig.height=5, dpi=300}
...
ggplot(deltaValY, aes(x=dobs, y=dpred, label=i_iso3, color=j_iso3)) +
  geom_point(size=1.5) +
...
ggplot(tauHMY, aes(x=i_iso3, y=j_iso3, fill=tau)) +
...
```{r trimai, echo=FALSE, warning=FALSE, message=FALSE, fig.cap = "Trade restrictiveness and market access conditions by country \\label{fig:trimai}", fig.height=6, dpi=300}

ggplot(data=trimaiY, aes(x=TRI, y=MAI, label=iso3)) +
  geom_point() +
  geom_text_repel() +
...
```{r maiGdppc, echo=FALSE, warning=FALSE, message=FALSE, fig.cap = "Market access conditions and per capita national income \\label{fig:maiGdppc}", fig.height=5, dpi=300}

ggplot(data=trimaiY, aes(x=gdppc, y=MAI, label=iso3))  +
  geom_point() +
  geom_text_repel() +
  geom_smooth(method='lm', lty=2, se=FALSE) +
  xlab('GDP Per Capita') +
...
```{r seaModel, echo=FALSE, warning=FALSE, message=FALSE, results="asis"}

stargazer(seaModelOutput, header=FALSE, type='latex', omit=c("hs2", "Constant", "year"), omit.stat=c("adj.rsq", "f", "ser"), add.lines = list(c("Product fixed effects?", "\\checkmark"), c("Cubic time spline?", "\\checkmark")), dep.var.labels = c("Freight Cost"), covariate.labels = c("CERDI seadist (std)", "Contiguity"), title = "Maritime Cost Model")
...
```{r landModel, echo=FALSE, warning=FALSE, message=FALSE, results="asis"}

stargazer(landModelOutput, header=FALSE, type='latex', omit=c("hs2", "Constant", "year"), omit.stat=c("adj.rsq", "f", "ser"), add.lines = list(c("Product fixed effects?", "\\checkmark"), c("Cubic time spline?", "\\checkmark")), dep.var.labels = c("Freight Cost"), covariate.labels = c("CEPII distw (std)", "Contiguity"), title = "Land Cost Model")
...
```{r airModel, echo=FALSE, warning=FALSE, message=FALSE, results="asis"}

stargazer(airModelOutput, header=FALSE, type='latex', omit=c("hs2", "Constant", "year"), omit.stat=c("adj.rsq", "f", "ser"), add.lines = list(c("Product fixed effects?", "\\checkmark"), c("Cubic time spline?", "\\checkmark")), dep.var.labels = c("Freight Cost"), covariate.labels = c("CEPII distw (std)", "Contiguity"), title = "Air Cost Model")
...
ccodes <- read_csv("estimation/clean/ccodes.csv") %>% pull(.)
