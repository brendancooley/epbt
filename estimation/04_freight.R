...
isl$island <- ifelse(isl$ccode %in% c("AUS", "JPN", "IDN", "PHL"), 1, 0)
...
seaModelOutput <- lm(avcsea ~ hs2 + bs(year, degree = 3) + seadist_scaled + contig, data=flowsSea, weights=flowsSea$val)
...
landModelOutput <- lm(avcland ~ hs2 + bs(year, degree=3) + adist_scaled + contig, data=flowsLand, weights=flowsLand$val)
...
airModelOutput <- lm(avcair ~ hs2 + bs(year, degree=3) + adist_scaled + contig, data=flowsAir, weights=flowsAir$val)
...
flowsModesX <- flowsModes %>% select(adist, seadist, contig, year, i_island, j_island, hs2)
flowsModesXOutput <- flowsModes %>% select(adist_scaled, seadist_scaled, contig, year, i_island, j_island, hs2)
flowsModesY <- flowsModes %>% select(airshare, seashare, landshare, othershare) %>% as.matrix()

d <- 0
modesModelOutput <- multinom(flowsModesY ~ hs2 + bs(year, degree=3) + adist_scaled + seadist_scaled + contig + i_island + j_island, data=flowsModesXOutput, decay=d, trace=FALSE)

if (runPreds == TRUE) {
  
  seaModel <- lm(avcsea ~ hs2 + bs(year, degree = 3) + seadist + contig, data=flowsSea, weights=flowsSea$val)
  landModel <- lm(avcland ~ hs2 + bs(year, degree=3) + adist + contig, data=flowsLand, weights=flowsLand$val)
  airModel <- lm(avcair ~ hs2 + bs(year, degree=3) + adist + contig, data=flowsAir, weights=flowsAir$val)
  modesModel <- multinom(flowsModesY ~ hs2 + bs(year, degree=3) + adist + seadist + contig + i_island + j_island, data=flowsModesX, decay=d, trace=FALSE)
  
  ### MAP AD VALOREM COST PREDICTIONS
  
  flowsSeaX <- flows %>% select(hs2, year, seadist, contig, i_island, j_island)
  flows$avcseaP <- predict(seaModel, flowsSeaX)
  
  flowsAirX <- flows %>% select(hs2, year, adist, contig)
  flows$avcairP <- predict(airModel, flowsAirX)
  
  flowsLandX <- flows %>% select(hs2, year, adist, contig)
